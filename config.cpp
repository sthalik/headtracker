...
static const ht_reflection_t ht_reflection_info[] = {
    ...
    F(pyrlk_win_size_w, int, 31, 4, 60,
    "Window size for optical flow tracking, vertical"),
    F(pyrlk_win_size_h, int, 31, 3, 45,
    "Window size for optical flow tracking, horizontal."),
    F(ransac_posit_eps, float, 1e-2, 1.0e-10, 1.0e5,
    "cvPOSIT epsilon for the purpose of estimating a feature set in RANSAC."),
    F(ransac_posit_iter, int, 6, 10, 200,
    "cvPOSIT iteration count for the purpose of estimating a feature set in RANSAC."),
    F(max_keypoints, int, 200, 30, 400,
    "Maximum keypoints to track"),
    F(keypoint_quality, int, HT_FEATURE_MIN_QUALITY_LEVEL, 10, 60,
    "Starting keypoint quality"),
    F(keypoint_distance, float, 3.6f, 1.5f, 50.0f,
    "Minimum Euclidean distance between keypoints"),
    F(keypoint_3distance, float, 11.0f, 1.5f, 50.0f,
    "Minimum Euclidean distance between keypoints"),
    F(keypoint_10distance, float, 25.0f, 1.5f, 50.0f,
    "Minimum Euclidean distance between keypoints"),
    ...
    F(ransac_num_iters, int, 33, 5, 100,
    "RANSAC iterations per frame"),
    F(ransac_max_error, float, 5, 2.0, 50.0,
    ""),
    F(ransac_avg_error, float, 1.0, 0.0, 1.0,
    ""),
    F(pyrlk_min_eigenval, float, 1.0e-4, 1.0e-10, 1.0e-1,
    "Min eigenval for Lukas-Kanade"),
    F(max_best_error, float, 4.5, 4, 1000,
    "Max RANSAC error"),
    F(debug, bool, true, 0, 1),
    F(ransac_max_threads, int, 3, 1, 256,
    "Max threads for RANSAC"),
    F(ransac_min_features, float, 0.3, 0.1, 1.0,
