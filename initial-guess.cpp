...
bool ht_initial_guess(headtracker_t& ctx, Mat& frame, float* rotation_matrix, float* translation_vector) {
    ...
    object_points[HT_CLASSIFIER_EYE1-1] = cvPoint3D32f(-23.23, -29, -31);
    object_points[HT_CLASSIFIER_EYE2-1] = cvPoint3D32f(23.23, -29, -31);
    object_points[HT_CLASSIFIER_MOUTH-1] = cvPoint3D32f(0, 29, -20);
	...
	return ht_posit(image_points,
					...
					rotation_matrix,
					translation_vector,
                    cvTermCriteria(CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 500, 1.0e-6),
					ctx.focal_length);
}
