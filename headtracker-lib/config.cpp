...
static const ht_reflection_t ht_reflection_info[] = {
    ...
    F(camera_index, int, CV_CAP_ANY, -1, 100000,
    "Choose a different camera by its platform-specific index."),
    F(ransac_num_iters, int, 10, 5, 100,
    "RANSAC iterations per frame"),
    F(ransac_smaller_error_preference, float, 0.05, 0.0, 1.0f,
    "How much to bias toward smaller reprojection error"),
    F(ransac_max_error, double, 0.96f, 0.0f, 1.0f,
    ""),
    F(ransac_avg_error, double, 0.92f, 0.0f, 1.0f,
    ""),
    F(pyrlk_min_eigenval, double, 1e-4, 1.0e-10, 1.0e-1,
    "Min eigenval for Lukas-Kanade"),
    F(max_best_error, double, 12, 10, 1000,
    "Max RANSAC error"),
    F(debug, bool, true, 0, 1),
    F(ransac_max_threads, int, 4, 1, 256,
    "Max threads for RANSAC"),
    F(ransac_min_features, int, 20, 10, 100,
