...
start_keypoints:
    int good = 0;
    if (ctx.keypoint_count < ctx.config.max_keypoints) {
        ORB detector = ORB(ctx.config.max_keypoints * 20, 1.2f, 8, ctx.config.keypoint_quality, 0, 2, ORB::HARRIS_SCORE, ctx.config.keypoint_quality);
